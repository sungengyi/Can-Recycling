...
public class OdometerData {
  ...
  private volatile boolean isResetting = false; // Indicates if a thread is
                                               // trying to reset any
                                               // position parameters
  private Condition doneResetting = lock.newCondition(); // Let other threads
                                                        // know that a reset
                                                        // operation is
                                                        // over.
  ...
  public double[] getXYT() {
    ...
    try {
      while (isResetting) { // If a reset operation is being executed, wait
        // until it is over.
        doneResetting.await(); // Using await() is lighter on the CPU
  ...
  public void update(double dx, double dy, double dtheta) {
    ...
    try {
      ...
      theta = (theta + (360 + dtheta) % 360) % 360; // keeps the updates
                                                    // within 360
                                                    // degrees
      isResetting = false; // Done resetting
      doneResetting.signalAll(); // Let the other threads know that you are
                                // done resetting
    } finally {
      lock.unlock();
    }
